...
[RPG_Common](https://github.com/hechuangguo/RPG_Common) 是 Server 与 Client **共用**的客户端 wire 协议独立仓库。双方通过 Git Submodule 挂载到各自主仓库的 `Common/` 目录，保证 `ClientMsg.h` 等同源同版本。
...
| 文件 | 说明 |
|------|------|
| `ClientMsg.h` | `ClientModule`、`ClientMsgID`、`Msg_C2S_*` / `Msg_S2C_*` 结构体 |
...
`autoinit.sh` 会自动执行 `git submodule update --init --recursive` 并校验 `Common/ClientMsg.h` 存在。
...
# 编辑 ClientMsg.h / NetDefine.h / MsgId.h
git add .
...
sequenceDiagram
    ...
    Note over DevC: 获得最新 ClientMsg.h
...
引用方式：`#include "ClientMsg.h"`（或按工程约定使用相对路径）。
...
| 现象 | 原因 | 处理 |
|------|------|------|
| `Common/` 为空或缺少 `ClientMsg.h` | clone 时未拉 submodule | `git submodule update --init --recursive` |
...
**禁止**：在 Server 或 Client 主仓库内手抄一份 `ClientMsg.h`；所有协议变更只通过 RPG_Common 流转。
