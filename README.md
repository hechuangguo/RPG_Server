...
├── Protobuf/      # protoc C++ 生成物（scripts/gen_proto.sh；*.pb.* gitignore）
├── script/        # Lua 游戏脚本
...
├── scripts/       # check_common_*.sh、gen_proto.sh
├── gen_data.sh    # 配表生成入口
...
`Common/` 为 [RPG_Common](https://github.com/hechuangguo/RPG_Common) 的 **Git Submodule**（与 Client 独立仓库 RPG_Client 的 `Common/` 同源）：
