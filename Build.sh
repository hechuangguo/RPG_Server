...
DO_REBUILD=false
TARGETS=()                       # 空表示构建全部目标
NEED_CONFIGURE=false             # proto/生成物变更后需重新 cmake
...
gen_proto() {
    ...
    info "生成 Protobuf (Common/*.proto → Protobuf/)..."
    chmod +x "${SCRIPT_DIR}/scripts/gen_proto.sh" 2>/dev/null || true
    if "${SCRIPT_DIR}/scripts/gen_proto.sh"; then
...
mark_configure_needed_if_proto_changed() {
    ...
    if find "${SCRIPT_DIR}/Common" -maxdepth 1 -name '*.proto' -newer "${cache}" -print -quit | grep -q .; then
        return
    fi
    if find "${SCRIPT_DIR}/Protobuf" -name '*.cc' -newer "${cache}" -print -quit 2>/dev/null | grep -q .; then
        return
    fi
