...
**改造重点：** Unity 新客户端 + **Protobuf 协议迁移** + 地图导出管线 + Scene 地图/移动校验；**不推倒**进程拓扑与 4 字节帧路由头。
...
- 自定义 **TCP + TLS** + **Protobuf body** 对接 4 字节帧路由
...
**保留** 4 字节帧头用于 Gateway 路由；**仅 body** 改为 Protobuf 二进制。服间协议（`protocal/InternalMsg.h`）暂维持现状，后续可按需单独迁移。
...
- `bodyLen` = **Protobuf 序列化字节数**（不含 4 字节头）
...
├── ClientCommon.proto               # ClientModule、Vec3、ProtocolVersion
├── WireCommon.proto                 # 帧常量、WireMsgHeader、FlatMsgId 约定
...
| 文件 | 职责 |
|------|------|
| `ClientCommon.proto` `ClientModule` | module 编号（4 字节帧路由头） |
| `WireCommon.proto` | 帧缓冲区上限与 WireMsgHeader 逻辑结构 |
...
1. **`ClientCommon.proto`** — 若新域，补 `ClientModule`（module 编号不变）
...
| `RpgProtocolFramer.cs` | 4 字节头拆包/组包 |
...
> 红线：角色主写在 RecordServer；Login/Super/Session 各连对应 MySQL 库；AOI 在 AOIServer；客户端上行经 Gateway 校验；handler 内不阻塞 IO。
