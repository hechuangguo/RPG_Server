...
客户端与服间 TCP **共用** 4 字节消息头（`bodyLen` + `module` + `sub`），定义于 [`sdk/net/NetDefine.h`](../sdk/net/NetDefine.h)。  
...
| 0x00 | 0x10 | S2C_LOGIN_CHALLENGE | S→C | `rpg::login::S2CLoginChallenge` | LoginServer | **连接后首包**；16 字节 `nonce`，`C2S_LOGIN_REQ.login_nonce` 须原样回显 |
| 0x00 | 0x02 | S2C_LOGIN_RSP | S→C | `rpg::login::S2CLoginRsp` | LoginServer | **仅 9010 账号登录**结果（含 token） |
| 0x00 | 0x11 | S2C_GATEWAY_AUTH_RSP | S→C | `rpg::login::S2CGatewayAuthRsp` | GatewayServer | Gateway 票据鉴权结果 |
| 0x00 | 0x12 | S2C_ENTER_WORLD_RSP | S→C | `rpg::login::S2CEnterWorldRsp` | GatewayServer | 选角进世界结果；成功时另发 `S2C_ENTER_GAME` |
...
| 5 | INVALID_MODEL | 角色模型 ID 非法（须 1–4） |

`C2S_CREATE_USER_REQ` 字段：`name`、`vocation`、`sex`、**`model_id`**（1=男大 2=男小 3=女大 4=女小）。`S2C_USER_LIST.UserListEntry` 与 `S2C_ENTER_GAME` 均含 `model_id`。
...
| `Msg_GW_SendToClient` | GW_SEND_TO_CLIENT (0x1402) | Scene/Session → Gateway | 同上，Gateway 组 4 字节头发客户端 |
...
sequenceDiagram
    ...
    GW-->>C: S2C_ENTER_WORLD_RSP + S2C_ENTER_GAME
