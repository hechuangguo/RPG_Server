#!/usr/bin/env bash
# ============================================================
#  push.sh —— 一键提交并推送主仓库与 Common 子模块到 GitHub
#
#  用法：
#    ./push.sh -m "提交说明"
#
#  流程：
#    1. Common（RPG_Common）：add -A → commit →（若 detached HEAD 则合并到 main）→ push
#    2. 主仓库（RPG_Server）：add -A → commit → push
#
#  说明：
#    - 仅支持 -m 参数；-m 为必填
#    - 提交并推送两个仓库中的全部改动（含未跟踪文件，受 .gitignore 约束）
#    - Common HTTPS push 失败时自动切换 SSH 重试
# ============================================================
...
[[ -n "${MAIN_BRANCH}" ]] || err "主仓库处于 detached HEAD，请先 checkout main"
...
git_push() {
    ...
    if [[ "${repo}" == "Common" ]]; then
        ...
        if [[ "${url}" == https://github.com/hechuangguo/RPG_Common.git ]]; then
            warn "${label} HTTPS push 失败，切换 SSH 重试..."
            git -C Common remote set-url origin git@github.com:hechuangguo/RPG_Common.git
            git -C Common push origin "${branch}" && return 0
        fi
    fi

    err "${label} push 失败，请先 ./pull.sh 解决冲突后再试"
}
...
commit_and_push_common() {
    ...
    if ! git -C Common symbolic-ref -q HEAD &>/dev/null; then
        detached=true
        warn "Common 处于 detached HEAD，将先提交再合并到 main"
    fi
...
commit_and_push_main() {
    ...
    if [[ "${ahead}" -gt 0 ]]; then
        ...
        git push origin "${MAIN_BRANCH}" || err "主仓库 push 失败，请先 ./pull.sh 解决冲突后再试"
        ok "主仓库已推送"
    else
        info "主仓库已与远程同步"
    fi
