...
[RPG_Common](https://github.com/hechuangguo/RPG_Common) 是 Server 与 Client **共用**的客户端 wire 协议独立仓库。双方通过 Git Submodule 挂载到各自主仓库的 `Common/` 目录，保证各域 `*.proto` 同源同版本。
...
| 文件 | 说明 |
|------|------|
| `ClientCommon.proto` | 全局 `ClientModule`、`Vec3`、`ProtocolVersion` |
| `WireCommon.proto` | 帧缓冲区上限、`WireMsgHeader`、FlatMsgId 约定 |
...
Server 运行时 C++ 帧 struct 与 `makeMsgId` 等 helper 在 [`sdk/net/NetDefine.h`](../sdk/net/NetDefine.h)、[`sdk/net/MsgId.h`](../sdk/net/MsgId.h)；数值须与 `WireCommon.proto` 一致。
...
flowchart TB
    subgraph RPG_Common_repo [RPG_Common]
        ClientCommon[ClientCommon.proto]
        WireCommon[WireCommon.proto]
...
`autoinit.sh` 会自动执行 `git submodule update --init --recursive` 并校验 `Common/ClientCommon.proto` 与 `Common/LoginMsg.proto` 存在。
...
# 编辑 XxxCommon.proto / XxxMsg.proto / ClientCommon.proto / WireCommon.proto（含字段注释）
git add .
...
引用方式：Server 链接 `Protobuf/*.pb.h`（CMake include）；路由枚举用 `ClientCommon.pb.h`（`rpg::client::ClientModule`）。Client 自行从 `.proto` 生成。
...
| 现象 | 原因 | 处理 |
|------|------|------|
| `Common/` 为空或缺少 `ClientCommon.proto` | clone 时未拉 submodule | `git submodule update --init --recursive` |
