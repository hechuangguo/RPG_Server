...
架构概览见 [ARCHITECTURE.md](ARCHITECTURE.md)；外联四服见 [EXTERNAL.md](EXTERNAL.md)。
...
| 消息 | 处理 |
|------|------|
| GW_SEND_TO_CLIENT | 组 6 字节头下发客户端 |
| GW_KICK_CLIENT | 强制断连 |
| REC_LOGIN_VERIFY_RSP | → GW_USER_LOGIN_REQ |
| GW_USER_LOGIN_RSP | S2C_LOGIN_RSP + S2C_ENTER_GAME，绑定 sceneServerId |
...
| 消息 | 处理 |
|------|------|
| REC_LOGIN_VERIFY_REQ | 按 CharBase.name 查找/创建；password 未校验 |
...
| **入站** | Gateway（GW_CLIENT_MSG）、Scene（场景/副本） |
| **出站** | Super、Record |
...
| **入站** | ClientListen（9010）、RegisterListen（19010，Super 代理 Gateway） |
| **出站** | 无 |
| **配置** | `LoginServer/extern_login.xml` |
...
| **配置** | `GlobalServer/extern_global.xml` |
