...
| 0x00 | 0x05 | C2S_SELECT_USER_REQ | C→S | `rpg::login::C2SSelectUserReq` | 已实现 | 选角进世界 |
| 0x00 | 0x06 | S2C_USER_LIST | S→C | `rpg::login::S2CUserList` | 已实现 | repeated UserListEntry |
| 0x00 | 0x07 | C2S_CREATE_USER_REQ | C→S | `rpg::login::C2SCreateUserReq` | 已实现 | 创角 |
| 0x00 | 0x08 | S2C_CREATE_USER_RSP | S→C | `rpg::login::S2CCreateUserRsp` | 已实现 | 创角响应 |
| 0x00 | 0x0D | C2S_GATEWAY_AUTH_REQ | C→S | `rpg::login::C2SGatewayAuthReq` | 已实现 | Gateway 票据鉴权 |
